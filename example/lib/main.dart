...
class _MyAppState extends State<MyApp> {
                ...
                Row(
                  ...
                  children: [
                    Expanded(
                      child: ElevatedButton(
                        onPressed:
                            connected ? (() => this.printTest(false)) : null,
                    ...
                    Expanded(
                      child: ElevatedButton(
                        onPressed:
                            connected ? (() => this.printTest(true)) : null,
                        child: Text("Test PDF"),
                      ),
                    ),
                  ],
                ...
                Container(
                    ...
                    child: ListView.builder(
                      ...
                      itemBuilder: (context, index) {
                        return ListTile(
                          onTap: () {
                            String mac = items[index].macAddress;
                            this.connect(mac);
                          },
                          title: Text('Name: ${items[index].name}'),
                          subtitle:
                              Text("macAddress: ${items[index].macAddress}"),
                        );
                      },
                    )),
                SizedBox(height: 10),
  ...
  Future<void> printTest(bool pdf) async {
    /*if (kDebugMode) {
    ...
    if (conexionStatus) {
      ...
      if (Platform.isWindows) {
        ...
        List<int> ticket =
            pdf == false ? await testTicket() : await testPDFTicket();
  ...
  Future<PdfPageImage?> getImage() async {
    final document = await PdfDocument.openAsset('assets/Invoice_106.pdf');
  ...
  Future<List<int>> testPDFTicket() async {
    List<int> bytes = [];
